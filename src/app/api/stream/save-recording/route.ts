...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        if (sessionsRes.ok) {
          ...
          if (readySession?.playbackId) {
            ...
            // No ready session yet — recordings typically take 2–5 min to process.
            // Return 202 so the client can show a "try again in a moment" message
            // rather than saving a broken video row.
            const waitingCount = sessions.filter(s => s.recordingStatus === "waiting").length;
            console.log(
              `[save-recording] Recording not ready yet — ${waitingCount} session(s) still processing. Asking client to retry.`
            );
            return NextResponse.json(
              {
                error: "Recording is still processing",
                recording_processing: true,
                detail: "Livepeer needs a few more minutes to prepare the recording. Please try saving again shortly.",
              },
              { status: 202 }
            );
          }
    ...
    // Use the VOD CDN for recordings — livepeercdn.studio is the live-stream endpoint
    // and stops serving content as soon as the stream ends.
    const playbackUrl = `https://vod-cdn.lp-playback.studio/raw/jxf4iblf6wlsyor6526t4tcmtmqa/catalyst-vod-com/hls/${recordingPlaybackId}/index.m3u8`;

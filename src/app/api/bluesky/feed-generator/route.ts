...
/**
 * GET /api/bluesky/feed-generator?uri={feedUri}&cursor={cursor}
 * Fetches posts from a Bluesky feed generator.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (!uri || !uri.startsWith("at://")) {
      return NextResponse.json({ error: "Valid feed AT-URI required" }, { status: 400 });
    }
    ...
    const res = await agent.app.bsky.feed.getFeed({ feed: uri, limit: 30, cursor });

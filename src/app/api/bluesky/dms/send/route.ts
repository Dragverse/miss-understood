...
export async function POST(request: NextRequest) {
  try {
    ...
    const { convoId, text } = await request.json();
    if (!convoId || !text?.trim()) {
      return NextResponse.json({ error: "convoId and text are required" }, { status: 400 });
    }
    ...
    console.error("[dms/send] Error:", error);

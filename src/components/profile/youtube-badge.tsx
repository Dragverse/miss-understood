...
export function YouTubeBadge({ channelId, channelName, className = "" }: YouTubeBadgeProps) {
  ...
  return (
    <a
      ...
      title={channelName ? `YouTube: ${channelName}` : "Connected to YouTube"}
      className={`inline-flex items-center gap-1 text-red-400 hover:text-red-300 transition group ${className}`}
    >
